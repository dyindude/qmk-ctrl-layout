node('master') {
  ...
  stage('Build/deliver layouts') {
    qmk.inside {
      sh 'cd qmk-firmware && make massdrop/ctrl:testlayout && make massdrop/ctrl:dyindude_md && mkdir -p /data/build/${JOB_NAME} && rm -f /data/build/${JOB_NAME}/* && cp *.bin /data/build/${JOB_NAME}/. -v'
    }
  }
}
